    ...
    push: async (joint: Joint, uqIn: UqBus, queue: number, data: any): Promise<boolean> => {
        ...
        if (source === 'self') {
            if (startDate * 1000 < Date.now() && endDate * 1000 > Date.now()) {
                await execSql(`if exists(select 1 from dbs.dbo.MGift where marketingId = @marketingId and jkcat = @sourceId)
                begin
                ...
                await execSql(`delete from dbs.dbo.MGift where marketingId = @marketingId and jkcat = @sourceId`, [
                    { 'name': 'marketingId', 'value': marketingId },
                    { 'name': 'sourceId', 'value': sourceId },
                ]);
            }
